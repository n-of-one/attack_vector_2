...
nohup java -jar app.jar --server.port=443 --server.ssl.key-store=file:keystore.p12 --server.ssl.key-store-password=password --server.ssl.key-store-type=pkcs12 --server.ssl.key-alias=tomcat --server.ssl.key-password=password >>logs.txt 2>>errors.txt &
#nohup java -jar app.jar --server.port=80 >>logs.txt 2>>errors.txt &
