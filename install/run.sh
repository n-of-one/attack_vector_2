...
# delete the entry that does not apply to your processor architecture.
sudo setcap CAP_NET_BIND_SERVICE=+eip /usr/lib/jvm/java-21-openjdk-arm64/bin/java
sudo setcap CAP_NET_BIND_SERVICE=+eip /usr/lib/jvm/java-21-openjdk-amd64/bin/java
...
JAR_FILE=$(ls attackvector-*.jar 2>/dev/null | head -1)
if [ -z "$JAR_FILE" ]; then
    echo "Error: No attackvector-*.jar file found in current directory"
    exit 1
fi

nohup java -jar "$JAR_FILE" --server.port=443 --server.ssl.key-store=file:keystore.p12 --server.ssl.key-store-password=password --server.ssl.key-store-type=pkcs12 --server.ssl.key-alias=tomcat --server.ssl.key-password=password >>logs.txt 2>>errors.txt &
#nohup java -jar "$JAR_FILE" --server.port=80 >>logs.txt 2>>errors.txt &
