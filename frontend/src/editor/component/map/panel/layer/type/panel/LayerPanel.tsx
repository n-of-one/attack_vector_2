...
import {AttributeLayerType} from "../../element/AttributeLayerType"
import {AttributeLayerLevel} from "../../element/AttributeLayerLevel"
    ...
    return (
        <div className="tab-content" id="node-layers-tab-content ">
            <div className="tab-pane active">
                <AttributeLayerType typeDisplay={typeDisplay} node={node} layer={layerData}/>
                <AttributeLayerLevel layer={layerData} node={node}/>
                <TextAttribute key={key("id")} size="small" label="Layer id" value={layerObject.id} readOnly={true}
                               help="Unique ID of this layer. Used when layers refer to each other." id="layerId"/>
                <TextAttribute key={key("name")} size="small" label="Layer name" value={layerObject.name} save={value => layerObject.saveName(value)}
                ...
                {children}
                <TextAttribute key={key("note")} size="large" label="Note" value={layerObject.note} save={value => layerObject.saveNote(value)}
