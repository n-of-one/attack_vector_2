...
import {SERVER_ENTERED_RUN, SERVER_LAYER_CHANGED, SERVER_LAYER_HACKED} from "../../RunServerActionProcessor";
...
export const siteReducer = (state: Site = defaultState, action: AnyAction): Site => {
    switch (action.type) {
        case SERVER_ENTERED_RUN:
            return action.data.site;
        case SERVER_LAYER_HACKED:
            return serverIceHacked(action.data, state);
        case SERVER_LAYER_CHANGED:
            return serverLayerChanged(action.data, state);
        default:
            return state;
    }
};
...
const serverIceHacked = (update: IceHackedData, site: Site) => {
    ...
    return {...site, nodes: newNodes};
};

interface LayerChangedData {
    nodeId: string,
    layer: LayerDetails
}

const serverLayerChanged = (update: LayerChangedData, site: Site) => {
    const node = findElementById(site.nodes, update.nodeId);
    const newLayers = updateArrayById(update.layer, node.layers, update.layer.id);
    const newNode = { ...node, layers: newLayers };
    const newNodes = updateArrayById(newNode, site.nodes, update.nodeId);
    return { ...site, nodes: newNodes };
}
