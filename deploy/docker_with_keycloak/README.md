...
To enabled https, un comment the content of [traefik_certificate_conf.yml](conf/traefik/conf.d/traefik_certificate_conf.yml)
Set the login, password and name of your domain name provider
...
3. Set `OpenId Connect: Url` using url to keycloak openid-connect apis (`http://auth.${DOMAIN}/realms/${KEYCLOAK_REALM_NAME}/protocol/openid-connect`)
